 ...
 * 
 * <p>
 * This service handles the setting and clearing of tenant context information
 * in both application memory (TenantContext) and PostgreSQL session variables.
 * It's responsible for ensuring proper isolation between different tenants
 * (groups)
 * by managing the database session state.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The service integrates with Spring Security to automatically determine
 * the current user's context and applies appropriate database session settings
 * to ensure data isolation through PostgreSQL RLS policies.
 * </p>
 * 
...
public class TenantContextService {
    ...
    /**
     * Sets the tenant context for the current user in both application memory and
     * database session.
     * 
     * <p>
     * This method configures PostgreSQL session variables that are used by Row
     * Level Security (RLS)
     * policies to ensure data isolation between different tenants. It handles
     * special cases:
     * </p>
     * <ul>
     * <li><strong>Group 0 (Pending Users):</strong> Users awaiting group approval
     * get restricted access</li>
     * <li><strong>Regular Groups:</strong> Normal tenant isolation is applied</li>
     * <li><strong>Null Group:</strong> Context is cleared for unauthenticated
     * scenarios</li>
     * </ul>
     * 
     * <p>
     * The method sets the following PostgreSQL session variables:
     * </p>
     * <ul>
     * <li>{@code app.current_user_id} - Current user's ID for audit purposes</li>
     * <li>{@code app.current_tenant_id} - Current tenant/group ID for RLS
     * policies</li>
     * <li>{@code app.group_id} - Group ID for backward compatibility</li>
     * </ul>
     * 
     * @param groupId the ID of the group/tenant to set as context. Can be null to
     *                clear context,
     *                0 for pending users, or any positive integer for regular
     *                groups
     * @throws IllegalStateException if no authenticated user is found in security
     *                               context
    ...
    /**
     * Sets the user context in database session without changing the current tenant
     * context.
     * 
     * <p>
     * This method is used to establish user identity in the database session while
     * preserving
     * any existing tenant context. If no tenant context exists, it automatically
     * initializes
     * the tenant context using the user's current group membership.
     * </p>
     * 
     * <p>
    ...
    /**
     * Clears all tenant context information from both application memory and
     * database session.
     * 
     * <p>
     * This method removes all tenant-related session variables from PostgreSQL and
     * clears
     * the application-level tenant context. It's typically called at the end of
     * request processing
     * to ensure clean state for subsequent requests.
     * </p>
     * 
     * <p>
     ...
     * 
     * <p>
     * <strong>Note:</strong> This operation should be performed with caution as it
     * affects
     * Row Level Security behavior for subsequent database operations in the same
     * transaction.
     * </p>
     * 
     * @see #setTenantContext(Integer)
    ...
    /**
     * Sets a special context for super administrators that bypasses Row Level
     * Security (RLS).
     * 
     * <p>
     * This method configures a special database session state that allows super
     * administrators
     * to access data across all tenants, effectively bypassing the multi-tenant
     * isolation.
     * It's used for administrative operations that require cross-tenant visibility.
     * </p>
     * 
     * <p>
     ...
     * <li>RLS policies should include specific clauses to handle SUPER_ADMIN
     * context</li>
     * </ul>
     * 
     * <p>
     * The method sets {@code app.current_tenant_id} to 'ADMIN' and clears the
     * application-level tenant context to signal RLS bypass mode.
     * </p>
     * 
    ...
    public void setSuperAdminContext() {
        ...
        if (currentUser != null && "ROLE_ADMIN".equals(currentUser.getRole())) {
            ...
            jdbcTemplate.execute("SET app.current_tenant_id = 'ADMIN'");
            jdbcTemplate.execute("SET app.group_id = 'ADMIN'");
     ...
     * 
     * <p>
     * This method provides access to the currently active tenant context without
     * requiring database queries. It's useful for validation and logging purposes.
     * </p>
     * 
     ...
     * 
     * <p>
     * Super administrators have special privileges including:
     * </p>
     * <ul>
     ...
     * </ul>
     * 
     * @return {@code true} if the current user is a super administrator,
     ...
     * 
     * <p>
     * This is a utility method that safely extracts the current user information
     * from the Spring Security authentication context. It handles cases where no
     * authentication is present or the principal is not a CustomUserDetails
     * instance.
     * </p>
     * 
     * @return the current authenticated user, or {@code null} if no user is
     *         authenticated
    ...
    private CustomUserDetails getCurrentUser() {
        try {
            ...
            // Authentication context might not be available (e.g., during startup, async
            // operations)
    ...
    /**
     * Automatically initializes the appropriate context based on the current user's
     * authentication.
     * 
     * <p>
     * This method is typically called during request processing to establish the
     * proper
     * tenant context. It determines whether to set regular tenant context or super
     * admin
     * context based on the user's role:
     * </p>
     * 
     * <ul>
     * <li><strong>Super Admin (ROLE_ADMIN):</strong> Sets super admin context with
     * RLS bypass</li>
     * <li><strong>Regular Users:</strong> Sets tenant context based on user's group
     * membership</li>
     * </ul>
     * 
     * <p>
     * This method is the primary entry point for context initialization and is
     * usually
     * called by security filters or interceptors.
     * </p>
     * 
     * @see #setTenantContext(Integer)
     * @see #setSuperAdminContext()
